...
example (φ : G →* H) (ψ : H →* K) (S : Subgroup G) : S.map (ψ.comp φ) = (S.map φ).map ψ := by
  ...
  ext w
  simp_rw [Subgroup.mem_map]
  simp only [exists_exists_and_eq_and]
  simp_rw [MonoidHom.comp_apply]
  done
